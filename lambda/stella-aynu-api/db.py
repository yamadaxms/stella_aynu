...
def to_text(value):
    ...
    if isinstance(value, (list, tuple)):
        return "、".join(text for text in (to_text(item) for item in value) if text)
            ...
            astro_links_by_culture.setdefault(row["star_culture_id"], []).append(
                {
                    ...
                    "link_memo": to_text(row["link_memo"]),
                }
            )
